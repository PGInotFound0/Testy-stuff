...
Production builds bake `SECURE_BOARD_HOMESERVER_URL` into `dist/config.json` after strict validation (explicit, non-loopback HTTPS); when the variable is unset, the build emits a placeholder and the container entrypoint (or a mounted `/config.json`) must supply the real URL at deployment time — the app refuses to start otherwise. The development `public/config.json` is ignored and cannot leak into a production build. The build also fails if a JavaScript chunk exceeds the explicit 1,200,000-byte budget; hashed assets should be served with immutable caching. The output in `dist/` is static. A self-hosted Synapse deployment should be a separate service and database, exposed over HTTPS — see `../synapse/` for the included one. Do not add secrets or access tokens to runtime config.
